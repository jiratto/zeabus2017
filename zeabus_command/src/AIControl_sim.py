...
class AIControl ():
	def __init__ (self):
		...
		self.stopTurn = True
		self.errDistance = 0.3
		# subscribe from /auv/state call back to set_position method
		rospy.Subscriber ('/syrena/state', Odometry, self.set_position)
	...
	def drive_x_rel (self, dis):
		...
		while not rospy.is_shutdown ():
			...
			if diff_total <= abs (dis):
				vx = ((dis / diff_total) / 10) * direction
				self.drive_xaxis (vx)
				rospy.sleep (0.5)
	...
	def drive_y_rel (self, dis):
		...
		while not rospy.is_shutdown ():
			...
			if diff_total <= abs (dis):
				vy = ((dis / diff_total) / 10) * direction
				self.drive_yaxis (vy)
				rospy.sleep (0.5)
				print ('present dist: ', diff_total)
...
if __name__ == '__main__':
	rospy.init_node('AIcontrol_sim')
	aicontrol = AIControl ()
	aicontrol.drive_x_relative(10,-25,0)