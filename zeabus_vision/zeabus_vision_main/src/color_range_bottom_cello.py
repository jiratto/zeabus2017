...
def select_color():
    ...
    cv2.namedWindow('imageBGR', flags=cv2.WINDOW_NORMAL)
    cv2.moveWindow('imageBGR', 400, 400)
    cv2.resizeWindow('imageBGR', (screenW / 3) + 30, (screenH / 3) + 30)
...
if __name__ == '__main__':
    rospy.init_node('color_range_bottom_cello')
    nodeName = 'color_range_bottom_cello'
    ...
    mission = rospy.get_param(nodeName + '/mission', 'default')
    ...
    if cameraPos == 'top':
        imageW = CONST.IMAGE_TOP_WIDTH
        imageH = CONST.IMAGE_TOP_HEIGHT
    else:
        imageW = CONST.IMAGE_BOTTOM_WIDTH
        imageH = CONST.IMAGE_BOTTOM_HEIGHT
