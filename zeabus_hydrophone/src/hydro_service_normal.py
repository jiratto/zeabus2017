...
#	    data = hydro_msg()
 #           data.distance = -999
  #          return data
    ...
    elv_t = xk[1]*180./pi
    c = xk[2]
    print "Az: %.2f,Elv: %.2f,Ot: %.3f ,Freq : %.0f" % (az_t, elv_t,c,fre/1000)    #Particle Filter 
    print "seq: %d,pct : %d,po1: %f,po2: %f,po3: %f,po4: %f) %(seq,pct,po1,po2,po3,po4)
    data = hydro_msg()
    data.azi = az_t
    data.elv = elv_t
...
def greet():
    ...
    rospy.Service('hydro', hydro_info, getData)
