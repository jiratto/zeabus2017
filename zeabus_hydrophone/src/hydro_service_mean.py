...
def getData_three(msg):
    ...
    if(tick_azi == 0 or tick_elv == 0):
        ...
        data.distance = 0  
        print "Mean azi : %f, Count azi : %f" %(data.azi,tick_azi)
        print "Mean elv : %f, Count elv : %f" %(data.elv,tick_elv)
    send_azi = [0]*3
    send_elv = [0]*3  
    return data
...
def getData(i):                                                                 
    ...
    v[0] = v[0]*pi/16.0 #16.0   #trace max speed 
    v[1] = v[1]*pi/5.0
    v[2] = v[2]*pi/3.0
    pf.predict(v)
    pf.update_weight(cp)
    ...
    send_azi[i]=az_t #np.abs(az_t)
    send_elv[i]=elv_t #np.abs(elv_t)
    
    
def greet():
    rospy.init_node('GUGU_IS_COMING_TO_TOWN')
    rospy.Service('hydro', hydro_info, getData_three)
    print "READY TO GET THE DATA"
    res = set(0x00,2000000)     #DelayObserve
    ser.write(res)
    res = set(0x01,28000)       #Frequency
    ser.write(res)
    res = set(0x02,0.30)        #FrontThreshold
    ser.write(res)
    res = set(0x03,0.02)        #PowerThreshold
    ser.write(res)
    res = set(0x04,1500)        #Soundspeed
    ser.write(res)  
    rospy.spin()
