...
def set(mode,value): #send to board 
    ...
    res.append('\xff')
    #res.append(mode)
    if mode==0x00 or mode==0x01 or mode==0x04, mode==0x05, mode==0x06, mode==0x07:
...
class readdata:
    ...
    def getData(self):
        ...
        if check_elv :    #Check elv_t < 25 three time? [N] --> check_elv = True
            data.stop = False
        i = 1
        while True: #attention here
            # print '0'
            #print ser
            x = ser.read(1) #boommmmmmm
            # print '1'
            # print x
            if x=='\xff':
                x = ser.read(1)
        ...
        seq = struct.unpack("<H", ''.join(x[2:4]))[0]    #H means short int type
        ...
        # print "c1r = %f || c1i = %f ||c2r = %f || c2i = %f ||c3r = %f || c3i = %f ||c4r = %f || c4i = %f" %(c1r,c1i,c2r,c2i,c3r,c3i,c4r,c4i)
        # print ""
...
if __name__ == '__main__':
    ...
    res = set(0x00,200)   #Delay observer (2000 mS)
    res = set(0x01,set_fre)  #Frequency
    res = set(0x02,0.23)    #Front Threshold (Volt) @night 16-17 july 2017 |40k = 0.23| |35k = 0.25|
    res = set(0x03,0.02)   #Power Threshold (dB)   
    res = set(0x04,1496)   #Acoustic sound speed (1500 m/S)
